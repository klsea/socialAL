...
for i = 1:length(part)
     ...
     fname = join([socialAL, '/output/trial_estimates_common/', part{i}, '.csv']);
     rl = readtable(fname);
     ...
     clear j
     fname = join([socialAL, '/output/eventfiles/rl3/', part{i}, '/', part{i}, '_feedback_rpe.mat']);
     ...
     fname = join([socialAL, '/output/eventfiles/rl3/', part{i}, '/', part{i}, '_decision_prob.mat']);
     save(fname, 'names', 'durations', 'onsets', 'pmod');
     clear fname names durations onsets
end
